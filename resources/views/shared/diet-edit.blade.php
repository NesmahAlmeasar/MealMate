...
@section('title', 'تعديل حمية')

@section('content')
<div>
    <div class="page-header">
        <h1 class="page-title">تعديل حمية: {{ $diet->name }}</h1>
    </div>
    ...
    <div class="card form-container">
        <form action="{{ route('shared.diets.update', $diet->diets_id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                ...
                <div class="form-group">
                    ...
                    @if($diet->photo_url)
                        <div style="margin-top: 5px;">
                            <img src="{{ Storage::url($diet->photo_url) }}" alt="Current Photo" style="height: 50px; border-radius: 4px;">
                        </div>
                    @endif
                </div>
                ...
                <div class="form-group" style="grid-column: span 2;">
                    ...
                    <textarea id="description" name="description" class="form-control" rows="3" required>{{ old('description', $diet->description) }}</textarea>
                </div>
                ...
                <div id="clients-selection" style="grid-column: span 2; display: {{ old('is_public', $diet->is_public) ? 'none' : 'block' }};">
                    ...
                    @else
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 15px; max-height: 300px; overflow-y: auto; padding: 5px;">
                            ...
                            <label class="card" style="display: flex; align-items: center; gap: 15px; padding: 15px; cursor: pointer; transition: all 0.2s; border: 1px solid var(--border-color); box-shadow: none; {{ in_array($client->clients_id, old('clients', $diet->clients->pluck('clients_id')->toArray())) ? 'background-color: var(--olive-light); border-color: var(--olive-medium);' : '' }}">
                                <input type="checkbox" name="clients[]" value="{{ $client->clients_id }}" 
                                       id="client_{{ $client->clients_id }}"
                                       {{ in_array($client->clients_id, old('clients', $diet->clients->pluck('clients_id')->toArray())) ? 'checked' : '' }}
                ...
                <div style="grid-column: span 2; margin-top: 20px;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 15px; max-height: 400px; overflow-y: auto; padding: 5px;">
                        @foreach($meals as $meal)
                            <label class="card" style="display: flex; align-items: start; gap: 10px; padding: 10px; cursor: pointer; border: 1px solid var(--border-color); box-shadow: none; {{ in_array($meal->meals_id, old('meals', $diet->meals->pluck('meals_id')->toArray())) ? 'background-color: var(--olive-light); border-color: var(--olive-medium);' : '' }}">
                                <input type="checkbox" name="meals[]" value="{{ $meal->meals_id }}" id="meal_{{ $meal->meals_id }}"
                                       {{ in_array($meal->meals_id, old('meals', $diet->meals->pluck('meals_id')->toArray())) ? 'checked' : '' }}
                ...
                <div style="grid-column: span 2; margin-top: 20px;">
                    ...
                    <div id="restrictions-container">
                        @forelse($diet->restrictions as $index => $res)
                        <div class="restriction-item" style="display: grid; grid-template-columns: 2fr 1.5fr 1fr 1.5fr auto; gap: 10px; align-items: end; background: var(--bg-gray); padding: 10px; border-radius: var(--radius-md); margin-bottom: 10px;">
                            ...
                            <div>
                                ...
                                <select name="restrictions[{{ $index }}][field_name]" class="form-control" required>
                                    ...
                                    <option value="calories" {{ $res->field_name == 'calories' ? 'selected' : '' }}>سعرات</option>
                                    <option value="protein_g" {{ $res->field_name == 'protein_g' ? 'selected' : '' }}>بروتين</option>
                                    <option value="carbs_g" {{ $res->field_name == 'carbs_g' ? 'selected' : '' }}>كربوهيدرات</option>
                                    <option value="fat_g" {{ $res->field_name == 'fat_g' ? 'selected' : '' }}>دهون</option>
                                </select>
                            </div>
                            <div>
                                 <label style="font-size: 0.8rem;">العملية</label>
                                 <select name="restrictions[{{ $index }}][operator]" class="form-control">
                                    <option value="<" {{ $res->operator == '<' ? 'selected' : '' }}><</option>
                                    <option value="<=" {{ $res->operator == '<=' ? 'selected' : '' }}><=</option>
                                    <option value="=" {{ $res->operator == '=' ? 'selected' : '' }}>=</option>
                                    <option value=">=" {{ $res->operator == '>=' ? 'selected' : '' }}>=></option>
                                    <option value=">" {{ $res->operator == '>' ? 'selected' : '' }}>></option>
                             ...
                             <div>
                                ...
                                <input type="number" name="restrictions[{{ $index }}][value]" class="form-control" value="{{ $res->value }}" placeholder="0">
                            </div>

                             <div>
                                <label style="font-size: 0.8rem;">الوصف (اختياري)</label>
                                <input type="text" name="restrictions[{{ $index }}][restriction]" class="form-control" value="{{ $res->restriction }}" placeholder="وصف القيد">
                            </div>
...
<script>
    // Initialize count based on existing restrictions
