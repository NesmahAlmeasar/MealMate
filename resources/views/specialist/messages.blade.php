...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        loadConversations().then(() => {
            // Check for user_id in URL to auto-open specific chat
            const urlParams = new URLSearchParams(window.location.search);
            const targetUserId = urlParams.get('user_id');
            
            if (targetUserId) {
                // Fetch specific user details to open chat immediately
                // This ensures it works even if the user isn't in the "Recent Conversations" list yet
        ...
        // Auto-refresh conversations list every 5 seconds (Optimized from 3)
    ...
    function loadConversations() {
        ...
        return fetch('{{ route("chat.conversations") }}')
            ...
            .then(users => {
                ...
                // If no users, show empty state (only if not already filtering or searching)
                if (users.length === 0) {
                ...
                list.innerHTML = ''; // Re-render list
                // Note: ideally we should diff the list, but full re-render is fine for < 50 items
                
                users.forEach(user => {
                    const div = document.createElement('div');
                    div.className = `conversation-item ${currentUserId == user.user_id ? 'active' : ''}`; // Use == for loose type check (string vs int)
