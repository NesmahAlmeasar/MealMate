...
<div>
        ...
        <div>
            ...
            @if(isset($selectedClient) && $selectedClient)
                <p style="margin: 5px 0 0 0; color: var(--olive-medium); font-size: 14px; display: flex; align-items: center; gap: 8px;">
                    ...
                    <span>إنشاء حمية خاصة لـ: <strong>{{ $selectedClient->user->Fname }} {{ $selectedClient->user->Lname }}</strong></span>
                </p>
            @endif
        </div>
    </div>
    ...
    <div class="card form-container">
        <form action="{{ route('specialist.diets.store') }}" method="POST" enctype="multipart/form-data">
            ...
            {{-- Hidden fields for consultation and client --}}
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                ...
                @if(isset($consultationId) && $consultationId)
                    <div class="form-group" style="grid-column: span 2;">
                        <div class="alert" style="background-color: var(--olive-light); color: var(--olive-dark); border: 1px solid var(--olive-medium);">
                            📌 ملاحظة: ستكون هذه الحمية <strong>خاصة</strong> بالمستخدم المحدد ومرتبطة بالاستشارة.
                        </div>
                    </div>
                @endif
                @endif

                {{-- Client Selection (only show if not from consultation) --}}
                @if(!isset($consultationId) || !$consultationId)
                ...
                @else
                {{-- If from consultation, auto-select the client --}}
